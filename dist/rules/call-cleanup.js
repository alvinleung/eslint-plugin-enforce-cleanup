...
exports.default = {
    ...
    create(context) {
        const externalImportList = [];
        return {
            ImportDeclaration(node) {
                const importSource = node.source.value; // Retrieve the import source
                const importTokens = [];
                // Only ignore external import, perform checks on internal
                if (isInternalImport(importSource)) {
                    // console.log(`External source detecetd, aborting...`, importSource);
                    return;
                }
                // Iterate over specifiers to get the tokens
                node.specifiers.forEach((specifier) => {
                    if (specifier.type === "ImportSpecifier") {
                        // For named imports (import { X as Y } from 'source')
                        importTokens.push(specifier.local.name);
                    }
                    else if (specifier.type === "ImportDefaultSpecifier") {
                        // For default imports (import X from 'source')
                        importTokens.push(specifier.local.name);
                    }
                    else if (specifier.type === "ImportNamespaceSpecifier") {
                        // For namespace imports (import * as X from 'source')
                        importTokens.push(specifier.local.name);
                    }
                });
                externalImportList.push(...importTokens);
            },
            ...
            NewExpression(node) {
                ...
                if (parentNode.type !== "PropertyDefinition" &&
                    ...
                    context.report({
                        node: node,
                        message: "Anonymous instance is discouraged as it is hard to cleanup",
                    });
                    return;
                }
                ...
                const isClassBelongToIngoredList = combinedIgnoreList.some((className) => className === instanceClassName);
                if (isClassBelongToIngoredList)
                    return;
                ...
                const hasCleanupCodeInFunction = findCleanupNodeRecursive(parentFunctionNode, instanceName);
                // early return, cleanup is called within the function
                if (hasCleanupCodeInFunction)
                    return;
                ...
                //   instanceName
                // );
                const hasCleanupCodeInClass = findCleanupNodeRecursive(parentClassNode, instanceName);
                if (hasCleanupCodeInClass)
                    return;
                context.report({
                    node: node,
...
function isInternalImport(importSource) {
    // Check if the import source is internal to the project
...
function findCleanupNodeRecursive(node, instanceName) {
    ...
    if (node.type === "CallExpression") {
        // For calls that without a target like just calling "subscribe" instead of "this.subscribe"
        const hasCallee = node.callee.property;
        if (!hasCallee)
            return false;
        // THIS IS THE EXIT CONDITION, WE FOUND THE "CALL", but we are not sure which
        // the node is now called
        const isCleanupFunctionCalled = node.callee.property.type === "Identifier" &&
            node.callee.property.name === "cleanup";
        const isCleanupCalledFromThisExpression = node.callee.type === "MemberExpression" &&
            node.callee.object.property &&
            node.callee.object.property.name === instanceName;
        const isCleanupCalledFromVar = node.callee.object.name === instanceName;
        return (isCleanupFunctionCalled &&
